# 🧪 Локальное тестирование загрузки APK

## ✅ Исправления ошибок компиляции

1. **http-exception.filter.ts** — исправлена ошибка типизации для `exception.stack`
2. **organizations.controller.ts** — дублирующий импорт уже удален (строка 20 закомментирована)

## 📋 Как проверить загрузку APK локально

### Вариант 1: Автоматический тест

```bash
./test-apk-locally.sh
```

**Что делает:**
- Проверяет, что backend запущен локально
- Копирует APK в `backend/app-debug.apk`
- Тестирует эндпоинт `/api/app/version`
- Скачивает APK и сравнивает размеры файлов

### Вариант 2: Вручную

#### Шаг 1: Запустить backend локально

```bash
cd backend
npm run start:dev
```

Backend должен запуститься на `http://localhost:3001`

#### Шаг 2: Скопировать APK в backend директорию

```bash
# Из корня проекта
cp ./apk-builds/app-debug-1.0.2.apk backend/app-debug.apk
```

Backend ищет файл по пути: `backend/app-debug.apk` (относительно `dist/main.js` это `../../app-debug.apk`)

#### Шаг 3: Проверить version endpoint

```bash
curl http://localhost:3001/api/app/version
```

**Ожидаемый ответ:**
```json
{
  "version": "1.0.2",
  "downloadUrl": "http://localhost:3001/app-debug.apk",
  "required": false,
  "releaseNotes": "Обновление включает новые функции и исправления ошибок."
}
```

#### Шаг 4: Скачать APK

**Через curl:**
```bash
curl -O http://localhost:3001/app-debug.apk
```

**В браузере:**
Откройте: http://localhost:3001/app-debug.apk

**Проверить размер:**
```bash
# Оригинал
ls -lh ./apk-builds/app-debug-1.0.2.apk

# Скачанный
ls -lh ./app-debug.apk

# Должны быть одинаковыми
```

## 🔍 Проверка в браузере

1. **Откройте:** http://localhost:3001/app-debug.apk
2. **Должно:** Начать скачивание файла `app-debug.apk`
3. **Проверьте:** Размер файла должен быть ~4.7 MB

## 📊 Что проверить

- [ ] Backend запущен локально
- [ ] APK файл скопирован в `backend/app-debug.apk`
- [ ] `/api/app/version` возвращает правильный `downloadUrl`
- [ ] `/app-debug.apk` доступен (HTTP 200)
- [ ] Файл скачивается полностью
- [ ] Размер скачанного файла совпадает с оригиналом

## 🐛 Решение проблем

### Проблема: "Backend is not running"

```bash
cd backend
npm install
npm run start:dev
```

### Проблема: "404 - Cannot GET /app-debug.apk"

**Причина:** Файл не скопирован в правильное место

**Решение:**
```bash
cp ./apk-builds/app-debug-1.0.2.apk backend/app-debug.apk
```

Затем перезапустить backend (Ctrl+C и `npm run start:dev`)

### Проблема: "APK file not found at: ..."

**Проверить путь в логах backend:**
```bash
# В логах backend должно быть:
# APK file path: /path/to/backend/app-debug.apk
```

**Исправить:** Скопировать файл в указанный путь

## 🎯 Полный тест локально

```bash
# 1. Запустить backend
cd backend && npm run start:dev &
BACKEND_PID=$!

# 2. Подождать запуска
sleep 5

# 3. Запустить тест
cd ..
./test-apk-locally.sh

# 4. Остановить backend
kill $BACKEND_PID
```

## 📝 Логи backend

При запуске backend локально вы увидите:
```
APK file path: /path/to/backend/app-debug.apk
Checking if APK exists at: /path/to/backend/app-debug.apk
✅ APK file found, size: 4948480 bytes
```

При запросе APK:
```
📥 APK download request: /app-debug.apk → /app-debug.apk
📁 Looking for APK at: /path/to/backend/app-debug.apk
✅ Serving APK file, size: 4948480 bytes
✅ APK file sent successfully
```

---

Готово к локальному тестированию! 🚀

