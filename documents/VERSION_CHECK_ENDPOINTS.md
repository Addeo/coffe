# 🔍 Эндпоинты для проверки обновлений

## 📡 Основной эндпоинт

### `GET /api/app/version`

**Описание:** Возвращает информацию о версии приложения на сервере.

**URL:** `http://192.144.12.102:3001/api/app/version`

**Метод:** `GET`

**Заголовки:** Не требуются (публичный эндпоинт)

**Ответ:**

```json
{
  "version": "1.0.2",
  "downloadUrl": "http://192.144.12.102:3001/app-debug.apk",
  "required": false,
  "releaseNotes": "Обновление включает новые функции и исправления ошибок."
}
```

**Поля ответа:**

- `version` (string) - Версия приложения на сервере
- `downloadUrl` (string) - URL для скачивания APK
- `required` (boolean) - Обязательное ли обновление (true = нельзя отменить)
- `releaseNotes` (string, опционально) - Заметки о релизе

---

## 🔄 Что происходит при запуске версии 1.0.1

### Шаг 1: При запуске приложения

```
AppComponent.ngOnInit()
  ↓
StartupService.checkForUpdates()
  ↓
AppUpdateService.checkForUpdates()
```

### Шаг 2: HTTP запрос

```typescript
// Код в AppUpdateService
const response = await http.get('http://192.144.12.102:3001/api/app/version');
```

**Что отправляется:**

```
GET http://192.144.12.102:3001/api/app/version
Headers:
  Accept: application/json
```

### Шаг 3: Получение ответа

```json
{
  "version": "1.0.2",
  "downloadUrl": "http://192.144.12.102:3001/app-debug.apk",
  "required": false,
  "releaseNotes": "Обновление включает новые функции и исправления ошибок."
}
```

### Шаг 4: Сравнение версий

```typescript
// Текущая версия в приложении: 1.0.1 (из build.gradle)
const currentVersion = '1.0.1'; // Из App.getInfo()
const serverVersion = response.version; // "1.0.2"

if (serverVersion !== currentVersion) {
  // Показать диалог обновления
}
```

### Шаг 5: Показ диалога (если версии не совпадают)

Пользователю показывается диалог:

- Заголовок: "Доступна новая версия"
- Версия: "1.0.2"
- Кнопки: "Обновить" / "Отмена"

---

## 🧪 Тестирование эндпоинта

### Через curl:

```bash
# Простой запрос
curl http://192.144.12.102:3001/api/app/version

# С форматированием JSON
curl -s http://192.144.12.102:3001/api/app/version | jq '.'

# Только версию
curl -s http://192.144.12.102:3001/api/app/version | jq '.version'
```

### Через скрипт:

```bash
./test-version-endpoint.sh
```

### Ожидаемый результат:

```json
{
  "version": "1.0.2",
  "downloadUrl": "http://192.144.12.102:3001/app-debug.apk",
  "required": false,
  "releaseNotes": "Обновление включает новые функции и исправления ошибок."
}
```

---

## 📋 Что проверит версия 1.0.1

### При запуске приложения:

1. **Проверка платформы:**

   ```typescript
   if (!Capacitor.isNativePlatform()) {
     // Веб-версия - проверка отключена
     return null;
   }
   ```

2. **Проверка интервала (24 часа):**

   ```typescript
   const lastCheck = localStorage.getItem('app_update_last_check');
   if (lastCheck && now - lastCheck < 24h) {
     // Пропустить проверку
     return null;
   }
   ```

3. **HTTP запрос:**

   ```typescript
   GET http://192.144.12.102:3001/api/app/version
   ```

4. **Сравнение версий:**

   ```typescript
   currentVersion = '1.0.1'; // Из AndroidManifest.xml
   serverVersion = '1.0.2'; // Из ответа API

   if (serverVersion !== currentVersion) {
     // Показать диалог
   }
   ```

5. **Проверка downloadUrl:**
   ```typescript
   if (!response.downloadUrl || response.downloadUrl.trim() === '') {
     // Ошибка - URL пустой
     return null;
   }
   ```

---

## 🔧 Логи в консоли приложения

### При успешной проверке:

```
🔍 Начинаем проверку обновлений...
📱 Платформа: android
🌐 Нативная платформа: true
📱 Текущая версия: 1.0.1
🌐 API URL: http://192.144.12.102:3001/api
🔍 Проверка обновлений. Текущая версия: 1.0.1
🌐 Запрос к: http://192.144.12.102:3001/api/app/version
📡 Получен ответ от сервера: {version: "1.0.2", ...}
✅ Доступна новая версия: 1.0.2
📥 URL для скачивания: http://192.144.12.102:3001/app-debug.apk
⚠️ Обязательное обновление: false
📢 Доступно обновление: 1.0.2
```

### Если версии совпадают:

```
🔍 Начинаем проверку обновлений...
📱 Текущая версия: 1.0.2
📡 Получен ответ от сервера: {version: "1.0.2", ...}
ℹ️ Обновления не требуются
```

### Если проверка недавняя:

```
⏰ Последняя проверка была 2 часов назад. Пропускаем.
```

---

## 🐛 Отладка проблем

### Проблема: Эндпоинт недоступен

```bash
# Проверить доступность
curl -v http://192.144.12.102:3001/api/app/version

# Проверить backend
ssh user1@192.144.12.102 "docker logs coffee_backend_fallback | tail -20"
```

### Проблема: Неправильный ответ

```bash
# Проверить код контроллера
cat backend/src/modules/app/app.controller.ts

# Проверить логи backend
ssh user1@192.144.12.102 "docker logs coffee_backend_fallback | grep 'app/version'"
```

### Проблема: downloadUrl недоступен

```bash
# Проверить APK на сервере
ssh user1@192.144.12.102 "ls -lh ~/coffe/app-debug.apk"

# Проверить доступность через HTTP
curl -I http://192.144.12.102:3001/app-debug.apk
```

---

## 📝 Полный пример запроса/ответа

### Запрос:

```http
GET /api/app/version HTTP/1.1
Host: 192.144.12.102:3001
Accept: application/json
User-Agent: Coffee-Admin/1.0.1 (Android)
```

### Ответ:

```http
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 145

{
  "version": "1.0.2",
  "downloadUrl": "http://192.144.12.102:3001/app-debug.apk",
  "required": false,
  "releaseNotes": "Обновление включает новые функции и исправления ошибок."
}
```

---

## ✅ Чеклист для проверки

- [ ] Эндпоинт `/api/app/version` доступен
- [ ] Возвращает корректный JSON
- [ ] Поле `version` содержит "1.0.2"
- [ ] Поле `downloadUrl` содержит рабочий URL
- [ ] URL для скачивания APK доступен (HTTP 200)
- [ ] Версия отличается от версии в приложении (1.0.1)
- [ ] Приложение показывает диалог обновления

---

## 🎯 Тестирование

```bash
# 1. Проверить эндпоинт
./test-version-endpoint.sh

# 2. Установить версию 1.0.1 на эмулятор
./install-v1-on-emulator.sh

# 3. Запустить приложение и проверить логи
./test-update.sh
```
