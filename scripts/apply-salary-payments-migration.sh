...
# ============================================
# Apply Salary Payments Migration to Production
# ============================================
...
MIGRATION_FILE="backend/migrations/012_create_salary_payments_table.sql"
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Apply Salary Payments Migration (Production)        ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
...
scp -o StrictHostKeyChecking=no "$MIGRATION_FILE" "${VPS_USER}@${VPS_HOST}:/tmp/012_salary_payments.sql"
...
ssh -o StrictHostKeyChecking=no "${VPS_USER}@${VPS_HOST}" << 'EOF'
  ...
  echo "Executing migration 012_create_salary_payments_table..."
  docker exec -i coffee_mysql_fallback mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" coffee_admin < /tmp/012_salary_payments.sql
  
  if [ $? -eq 0 ]; then
    echo "✅ Migration executed successfully"
    rm /tmp/012_salary_payments.sql
  else
...
ssh -o StrictHostKeyChecking=no "${VPS_USER}@${VPS_HOST}" << 'EOF'
  ...
  echo "Checking salary_payments table:"
  docker exec -i coffee_mysql_fallback mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" coffee_admin -e "
    SELECT COUNT(*) as table_exists FROM information_schema.tables 
    WHERE table_schema = 'coffee_admin' AND table_name = 'salary_payments';
  "
EOF
...
echo -e "${BLUE}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
echo "Table 'salary_payments' has been created successfully."
echo "User deletion should now work without the salary_payments error."
echo ""
