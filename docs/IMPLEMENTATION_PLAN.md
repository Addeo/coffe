# План изменений на основе анализа ответов
...
- Пример: 100 обычных + 50 сверхурочных с коэф. 1.6 = `100 + 50*1.6 = 180 часов`
...
2. **Order Entity** (`backend/src/entities/order.entity.ts`):
   - Заменить `overtimeHours` расчет на учет коэффициента
...
4. **EngineerOrganizationRate Entity** (`backend/src/entities/engineer-organization-rate.entity.ts`):
   - Заменить `customOvertimeRate` на `customOvertimeCoefficient: number`
   - Миграция данных из старого поля в новое

5. **Engineer Entity** (`backend/src/entities/engineer.entity.ts`):
   - Переименовать `overtimeRate` в `overtimeCoefficient`
   - Обновить тип с `decimal` на `decimal(5,2)` для коэффициента
...
2. **Components** - все места, где отображаются часы:
   - Обновить отображение: `regularHours + (overtimeHours * coefficient)`
   - Показать расчет в виде формулы для прозрачности

3. **Forms** - где редактируются ставки:
   - Поле "Ставка за сверхурочные" → "Коэффициент сверхурочных"
...
-- После проверки данных - удалить старые колонки
...
### 3. Определение сверхурочных часов
...
2. Добавить таблицу `holidays` для праздничных дней
3. В UI добавить автоматическое определение сверхурочных при создании WorkSession

---

### 4. Права доступа - пересмотр

**Критические изменения:**

1. **Админ не создает заявки** - убрать возможность создания заявок для ADMIN
   - `OrdersController.create()` - оставить только `UserRole.MANAGER`
   - `OrdersController.createAutomatic()` - удалить или оставить только MANAGER

2. **Менеджер может создавать автоматические заявки**
   - Обновить `@Roles(UserRole.MANAGER)` в `createAutomatic()`

3. **Инженер видит только свои заявки**
   - Обновить `OrdersService.findAll()` - автоматическая фильтрация по assignedEngineerId

4. **Менеджер видит статистику по часам всех инженеров, но не по заработку**
   - Разделить endpoints статистики:
     - `/statistics/hours` - доступен MANAGER
     - `/statistics/earnings` - только ADMIN

5. **Менеджер не может изменять ставки инженеров**
   - `EngineerOrganizationRatesController` - оставить только `UserRole.ADMIN`
...
2. При изменении ставки - сохранять в историю
3. При расчете оплаты - брать ставку, действовавшую на дату работы (`workDate`)

---

### 6. Множественное назначение инженеров на заявку
...
2. Обновить `Order.assignEngineer()` - добавлять в таблицу связи
3. Обновить статистику - учитывать всех инженеров по их WorkSession
4. Обновить UI - показывать список назначенных инженеров
...
2. **Frontend** - показывать только доступные роли для переключения
3. При logout - сбрасывать activeRole
...
### 9. Уведомление за 1 час до истечения срока
...
2. Настроить push-уведомления (Firebase Cloud Messaging или аналог)
...
4. Заявка остается в статусе WORKING
...
if (!customRate) {
  ...
  // ИЛИ возвращать коэффициент 0
}
```

**Нужно уточнить:** Что делать, если ставки не установлены - ошибка или предупреждение?
...
2. Если отработано больше плана:
   ...
   - Расчет: `(actualHours - planHours) * baseRate * overtimeCoefficient`
...
3. Убрать отслеживание `receivedFromOrganization` - не требуется
...
1. `/statistics/hours` - доступен MANAGER и ADMIN
   ...
   - Без данных о заработке

2. `/statistics/earnings` - только ADMIN
   - Полная статистика с заработком

3. `/statistics/my` - для USER
...
Пока оставить как есть, добавить в backlog.

---

### 20. Автоматические заявки

**Текущее состояние:** Не нужно реализовывать сейчас

**Действие:** Оставить endpoints, но пометить как "в разработке"
...
4. ✅ Права доступа (админ не создает заявки)
5. ✅ История ставок
...
1. **Отсутствие индивидуальных ставок:**
   - Коэффициент = 0 означает нулевую оплату?
   - Или нужно обязательно требовать установку ставок перед работой?

2. **Прибыль компании:**
   - Где хранятся ставки организаций для расчета прибыли?
   - Нужна ли таблица `organization_rates_history`?

3. **Экспорт данных:**
   - Какие форматы нужны?
   - Какие данные для каждой роли?

4. **Статистика "в работе":**
   - Требуется ли отдельная статистика по незавершенным заявкам?

5. **Автоматические заявки:**
   - Когда планируется реализация?
   - Какие интеграции ожидаются?
...
- [ ] Подготовить миграции для новых полей
- [ ] Создать тестовые данные для проверки новой логики
- [ ] Согласовать план с командой
- [ ] Определить даты релизов для каждой фазы
- [ ] Подготовить документацию по новой логике
...
5. ✅ Менеджер видит статистику по часам, но не по заработку
6. ✅ История ставок сохраняется и используется при расчете
7. ✅ Множественное назначение инженеров работает корректно
8. ✅ Фиксированная оплата за машину - раз в месяц
9. ✅ Доплаты за зоны - за каждый выезд
